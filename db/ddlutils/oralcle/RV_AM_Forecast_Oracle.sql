...
NULL AS AM_ServiceRequest_ID, NULL AS AM_RequestType_ID, am.AM_Pattern_ID,
am.Interval, 
am.Range, 
(CASE 
 	WHEN am.CalendarType = 'D' THEN adddays(am.DateNextRun, s.Serial)
	WHEN am.CalendarType = 'M' THEN add_months(am.DateNextRun, am.Interval)
 	WHEN am.CalendarType = 'R' THEN add_months(am.DateNextRun, s.Serial)
 	WHEN am.CalendarType = 'W' THEN addweeks(am.DateNextRun, s.Serial)
 	WHEN am.CalendarType = 'Y' THEN addyears(am.DateNextRun, s.Serial)
 ELSE NULL
END) AS DateNextRun,
0 AS Average,
0 Amt,
...
, (SELECT (RowNum - 1) AS Serial
    FROM DUAL
    CONNECT BY LEVEL <= 2000) s
WHERE am.ProgrammingType = 'C'
AND am.DocStatus <> 'IT'
AND am.IsActive = 'Y'
AND (CASE 
 	-- For Daily
 	WHEN am.CalendarType = 'D' AND am.IsSunday = 'Y' AND to_char(am.DateNextRun + s.Serial, 'D') = 1 THEN 'Y'
	WHEN am.CalendarType = 'D' AND am.IsMonday = 'Y' AND to_char(am.DateNextRun + s.Serial, 'D') = 2 THEN 'Y'
 	WHEN am.CalendarType = 'D' AND am.IsTuesday = 'Y' AND to_char(am.DateNextRun + s.Serial, 'D') = 3 THEN 'Y'
 	WHEN am.CalendarType = 'D' AND am.IsWednesday = 'Y' AND to_char(am.DateNextRun + s.Serial, 'D') = 4 THEN 'Y'
 	WHEN am.CalendarType = 'D' AND am.IsThursday = 'Y' AND to_char(am.DateNextRun + s.Serial, 'D') = 5 THEN 'Y'
 	WHEN am.CalendarType = 'D' AND am.IsFriday = 'Y' AND to_char(am.DateNextRun + s.Serial, 'D') = 6 THEN 'Y'
 	WHEN am.CalendarType = 'D' AND am.IsSaturday = 'Y' AND to_char(am.DateNextRun + s.Serial, 'D') = 7 THEN 'Y'
 	-- For MonthlyFixed
 	WHEN am.CalendarType = 'M' THEN 'Y'
  	-- For Other
 	WHEN am.CalendarType IN('R', 'W', 'Y') AND mod(s.Serial, am.Interval) = 0 THEN 'Y'
 ELSE 'N'
END) = 'Y'
AND NOT EXISTS(SELECT 1 FROM AM_ServiceOrder so 
               WHERE so.AM_Maintenance_ID = am.AM_Maintenance_ID
               AND trunc(so.DateStartPlan, 'DY') = trunc(adddays(am.DateNextRun, s.Serial), 'DY') 
               AND so.DocStatus IN('IP', 'CO', 'CL'))
UNION ALL
SELECT am.AM_Maintenance_ID, am.DocumentNo, am.Description, am.PriorityRule,   
a.A_Asset_ID, a.A_Asset_Group_ID, am.CalendarType, am.ProgrammingType, 
NULL AS AM_ServiceRequest_ID, NULL AM_RequestType_ID, am.AM_Pattern_ID,
am.Interval, am.Range, 
adddays(am.DateNextRun, s.Serial) AS DateNextRun,
mr.Average,
(mr.Amt + (mr.Average * s.Serial)) AS Amt,
am.AD_Client_ID, am.AD_Org_ID, am.IsActive, am.Created, am.CreatedBy, am.Updated, am.UpdatedBy
FROM AM_Maintenance am
INNER JOIN A_Asset a ON(a.A_Asset_ID = am.A_Asset_ID OR a.A_Asset_Group_ID = am.A_Asset_Group_ID)
INNER JOIN (SELECT m.AM_AssetMeter_ID, m.A_Asset_ID, m.Amt, 
           ...
           GROUP BY m.AM_AssetMeter_ID, m.A_Asset_ID, m.Amt) mr ON(mr.A_Asset_ID = a.A_Asset_ID)
, (SELECT (RowNum - 1) AS Serial
    FROM DUAL
    CONNECT BY LEVEL <= 2000) s
...
AND am.DocStatus <> 'IT'
AND am.IsActive = 'Y'
AND (mr.Amt + (mr.Average * s.Serial)) 
    BETWEEN ((am.Interval * ROUND((mr.Amt + (mr.Average * s.Serial)) / CASE WHEN am.Interval = 0 THEN 1 ELSE am.Interval END, 0)) - am.Range)
      AND ((am.Interval * ROUND((mr.Amt + (mr.Average * s.Serial)) / CASE WHEN am.Interval = 0 THEN 1 ELSE am.Interval END, 0)) + am.Range)
AND NOT EXISTS(SELECT 1 FROM AM_ServiceOrder so 
               WHERE so.AM_Maintenance_ID = am.AM_Maintenance_ID
               AND trunc(so.DateStartPlan, 'DY') = trunc(adddays(am.DateNextRun, s.Serial), 'DY') 
               AND so.DocStatus IN('IP', 'CO', 'CL'))

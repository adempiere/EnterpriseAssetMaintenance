...
public class MaintenanceValidator implements ModelValidator
{
	...
	private String UpdateServiceOrder(MInOut inout)
	{
		...
		for (MInOutLine ioline : lines)
		{
			...
			if (oline.get_ValueAsInt(X_AM_ServiceOrder_Resource.COLUMNNAME_AM_ServiceOrder_Resource_ID) != 0)
			{
				X_AM_ServiceOrder_Resource resource = new X_AM_ServiceOrder_Resource(inout.getCtx(),
						oline.get_ValueAsInt(X_AM_ServiceOrder_Resource.COLUMNNAME_AM_ServiceOrder_Resource_ID),
						inout.get_TrxName());
